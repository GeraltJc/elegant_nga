...
- `threads.reply_count_display` 为有效回复数口径（只增不减），用于展示与过滤。
  - 列表页无法解析回复数时不覆盖已有值，避免误写为 0。
  - 详情页可解析总回复数时优先使用，但仍只增不减。
  - 若详情页拿不到总楼层，仅在“已抓到末页”时用最大楼层号回填。
  - 后续列表页重复出现同主题时也不会把回复数回退。
  - 列表接口支持 `reply_min` / `reply_max` 参数按该口径过滤。
