...
> - `backend/database/migrations/2026_02_02_000001_add_audit_run_id_to_crawl_runs_table.php`
> - `backend/database/migrations/2026_02_02_000002_add_http_request_count_to_crawl_run_threads_table.php`
...
## crawl_runs（抓取运行记录（每次定时/手动任务的汇总审计））
...
| run_trigger_text | 触发来源（scheduler/manual/floor_audit） |
| audit_run_id | 关联缺楼层审计运行（thread_floor_audit_runs.id，可空） |
...
| http_request_count | 该主题处理期间的 HTTP 请求次数（含重试） |
| thread_id | 所属主题（threads.id） |
...
| run_trigger_text | 触发来源（scheduler/manual 等） |
