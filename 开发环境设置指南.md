...
- scheduler：常驻运行 Laravel Scheduler，按固定时间触发抓取与审计
...
- `CRAWLER_RUN_TIMES=03:00,10:00,15:00`（调度时间点口径）
- 抓取请求（网络与日志）：
...
- `NGA_CURL_LOG_PATH=...`（curl 日志路径，支持 `{date}` 占位；默认 `backend/storage/logs/nga-curl-YYYY-MM-DD.log`）
- `NGA_CURL_LOG_DAYS=3`（curl 日志保留天数，0 表示不自动清理）
- `NGA_GUEST_INIT_LOG_DAYS=3`（访客初始化日志保留天数，日志路径默认 `backend/storage/logs/nga-guest-init-YYYY-MM-DD.log`）
...
- 通过 docker compose 启动 `scheduler` 容器：运行 `php artisan schedule:work`
- 调度时间（Asia/Shanghai）：每天 `03:00 / 10:00 / 15:00`
- 实际执行命令：`nga:crawl-lite-and-audit`
  - 先执行 `nga:crawl-lite --fid=7 --recent-days=3`
  - 抓取成功后等待 5 分钟，再执行 `nga:audit-missing-floors --repair`
- 调度日志：`backend/storage/logs/scheduler-YYYY-MM-DD.log`（保留天数 `LOG_SCHEDULER_DAYS`，默认 14）

### 6.2 Cron（备选）

- 若必须使用 cron，可在容器内配置：每分钟触发一次 `php artisan schedule:run`
  - 仍由 Scheduler 内部控制 `03:00 / 10:00 / 15:00`

> 建议优先使用 6.1：更容易统一管理、也便于以后上云。
...
- 如需回放请求，可查看 `backend/storage/logs/nga-curl-YYYY-MM-DD.log`（可用 `NGA_CURL_LOG_PATH` 覆盖或置空禁用）
...
补充说明（测试环境）：
- `phpunit.xml` 已指定 SQLite 与 `APP_CONFIG_CACHE` 临时路径，测试会绕开 `bootstrap/cache/config.php`，避免误用 MySQL 配置。
...
- general_log 文件持续增长：排查结束后可关闭（`SET GLOBAL general_log = 'OFF'`）或做日志轮转
