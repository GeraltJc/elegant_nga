...
- “显示值”与“推导值”区分命名（如 `reply_count_display` 表示有效回复数，只增不减，用于展示与过滤）。
...
| reply_count_display | int unsigned | 有效回复数（只增不减，列表/详情纠正后用于展示与过滤） | 事实字段 |
...
- `reply_count_display`：有效回复数随回复增长（只增不减）。
- `last_seen_on_list_page_number`：主题热度变化导致页码位置变化。
- `is_pinned/is_digest`、标签映射：版务操作导致变化。
- `crawl_cursor_*`：增量抓取推进时持续变化。

**回复数口径补充**
- `reply_count_display` 为有效回复数口径：列表/详情抓取到更大的值时更新，缺失或更小值不覆盖。
- 用途：列表展示与回复数过滤统一依赖该字段，避免展示/筛选口径不一致。
...
| run_trigger_text | varchar(30) | `schedule_12h` / `manual` / `floor_audit` | 状态字段 |
| audit_run_id | bigint unsigned NULL | 关联缺楼层审计运行（thread_floor_audit_runs.id） | 关联字段 |
...
| http_request_count | int unsigned | 该主题处理期间的 HTTP 请求次数（含重试） | 状态字段 |
