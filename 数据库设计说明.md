...
- 增量策略：
  ...
  - 单主题抓取页上限：**5 页**；超过则标记截断。
- 存储：内容存 **清洗后的 HTML**；图片/附件仅存 URL。
- 编辑/删除/折叠：发现变更则更新当前内容，并写入历史版本表（含来源编辑时间）。
- Web 功能：列表（默认按发帖时间倒序，可切换按最后回复时间倒序）+ 分页（30/页）+ 搜索（标题 + 1 楼）+ 详情页 + 最近更新时间。
...
| list_url | varchar(255) | 版块列表 URL（排查用） | 事实字段 |
...
| is_truncated_by_page_limit | tinyint(1) | 是否因页上限截断 | 状态字段 |
| truncated_at_page_number | tinyint unsigned NULL | 截断发生页码（<=5） | 状态字段 |
| last_crawled_at | datetime NULL | 最近成功抓取时间 | 状态字段 |
| last_detected_change_at | datetime NULL | 最近一次检测到变化的时间 | 状态字段 |
| crawl_cursor_max_floor_number | int unsigned NULL | 已抓到最大楼层号（增量游标） | 状态字段 |
...
| floor_number | int unsigned | 楼层号（1 为首帖） | 事实字段 |
...
| change_detected_reason | varchar(60) | 变更原因（明确字符串，不用 status/type） | 状态字段 |
| crawl_run_thread_id | bigint unsigned NULL | 关联抓取明细（crawl_run_threads.id） | 关联字段 |

**change_detected_reason 建议白名单**
